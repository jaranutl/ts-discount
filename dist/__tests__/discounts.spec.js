...
describe('Discount engine – single campaigns', () => {
    test('Coupon: fixed amount 50 → final 550 (cartA)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [(0, discounts_1.fixedAmountCoupon)('c1', 50)]);
    ...
    test('Coupon: 10% → final 540 (cartA)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [(0, discounts_1.percentCoupon)('c2', 10)]);
        expect(res.finalTotal).toBe(540);
    });
    test('On Top: 15% off Clothing → final 2382.5 (cartB)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartB, [(0, discounts_1.percentOffCategory)('cat15', 'Clothing', 15)]);
    ...
    test('On Top: points 68, cap 20% → final 762 (cartC)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartC, [(0, discounts_1.pointsRedeem)('p1', 68, 20)]);
    ...
    test('Seasonal: every 300 get 40 → final 750 (cartC)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartC, [(0, discounts_1.thresholdEveryXGetY)('s1', 300, 40)]);
...
describe('Rule: only one campaign per Category (pick the best inside each)', () => {
    test('Among coupons, pick the higher discount (10% vs fixed 50 on cartA)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [
            (0, discounts_1.fixedAmountCoupon)('c-fixed-50', 50), // 50 off
            (0, discounts_1.percentCoupon)('c-10pct', 10) // 60 off → better
        ]);
    ...
    test('Among onTop (15% clothing vs points 100), pick larger discount for running total', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartB, [
            (0, discounts_1.percentOffCategory)('cat15', 'Clothing', 15), // 157.5 off
            (0, discounts_1.pointsRedeem)('pts100', 100, 20) // 100 off (cap is 508)
        ]);
        expect(res.lines).toHaveLength(1);
        expect(res.lines[0].kind).toBe('onTop');
...
describe('Order: Coupon → On Top → Seasonal', () => {
    test('Applies in strict order and uses running totals at each step', () => {
        ...
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [
            (0, discounts_1.percentCoupon)('cp10', 10),
            (0, discounts_1.pointsRedeem)('pts50', 50, 20),
            (0, discounts_1.thresholdEveryXGetY)('s300-40', 300, 40)
        ]);
    ...
    test('When multiple exist within a category, pick best THEN continue order', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartC, [
            // Coupons (best is 10% = 83 > fixed 50)
            (0, discounts_1.fixedAmountCoupon)('c50', 50),
            (0, discounts_1.percentCoupon)('c10', 10),
            // On Top (choose best between category 15% and points 60)
            (0, discounts_1.percentOffCategory)('cloth15', 'Clothing', 15), // 52.5 on T-Shirt 350
            (0, discounts_1.pointsRedeem)('pts60', 60, 20), // cap 20% of post-coupon
            // Seasonal
            (0, discounts_1.thresholdEveryXGetY)('s300-40', 300, 40)
        ]);
        // Validate order
...
describe('Edge cases & guards', () => {
    test('Fixed coupon cannot exceed running total or go negative', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [(0, discounts_1.fixedAmountCoupon)('cBig', 99999)]);
        expect(res.finalTotal).toBe(0);
    });
    test('Seasonal ignores non-positive parameters', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [
            (0, discounts_1.thresholdEveryXGetY)('bad1', 0, 40),
            (0, discounts_1.thresholdEveryXGetY)('bad2', 300, 0)
        ]);
...
describe('Additional tests', () => {
    test('Points redeem subtracts (no double-negative bug)', () => {
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [(0, discounts_1.pointsRedeem)('pts50', 50, 20)]);
        expect(res.subtotal).toBe(600);
        expect(res.lines).toHaveLength(1);
        expect(res.lines[0].kind).toBe('onTop');
    ...
    test('Points redeem is capped by capPercent of running total', () => {
        ...
        const res = (0, discounts_1.calculateFinalPrice)(cartB, [(0, discounts_1.pointsRedeem)('bigPts', 1000, 10)]);
        expect(res.lines).toHaveLength(1);
    ...
    test('Percent off category rounds to 2 decimals', () => {
        ...
        const res = (0, discounts_1.calculateFinalPrice)(cartB, [(0, discounts_1.percentOffCategory)('p-round', 'Clothing', 12.345)]);
        expect(res.lines).toHaveLength(1);
    ...
    test('When two coupons give the same discount, only one coupon line is applied', () => {
        ...
        const res = (0, discounts_1.calculateFinalPrice)(cartA, [
            (0, discounts_1.fixedAmountCoupon)('fix60', 60),
            (0, discounts_1.percentCoupon)('pct10', 10)
        ]);
    ...
    test('Seasonal applies correctly after coupons and onTop (order preserved)', () => {
        ...
        const res = (0, discounts_1.calculateFinalPrice)(cartB, [
            (0, discounts_1.percentCoupon)('c10', 10), // 254 off -> 2286 running
            (0, discounts_1.percentOffCategory)('cloth20', 'Clothing', 20), // clothing base 1050 -> 210 off
            (0, discounts_1.pointsRedeem)('pts500', 500, 50), // huge points but cap 50% of running (1143) -> capped, not chosen
            (0, discounts_1.thresholdEveryXGetY)('s500-30', 500, 30)
        ]);
